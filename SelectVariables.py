# -*- coding: utf-8 -*-
"""
Created on Tue Dec 28 14:58:50 2021
...
from tkinter import ttk, Scrollbar, StringVar
import TreviewAdriana as tvadr
import coeficienteVariacao as coefvar
import algoritmo_critic as algcritic
...
def novajanela(root, df):
    ...
    SVariable.title("Selecionar variáveis ADRIANA")
    ...
    def wCritic():
        lista_var = pvar()
        pesos = algcritic.wPesosCritic(df, lista_var)
        Npesos.delete(0,END)
        Npesos.insert("end", *pesos)
    ...
    btnCritic = Button(SVariable, text="CRITIC ", 
                        fg = 'black', bg = 'White', font = 'time 8 bold',
                        command=lambda: wCritic())
    btnCritic.place(rely=0.5, relx=0.6)  
    
    btnMadr = Button(SVariable, text="Método Adriana", 
                        fg = 'black', bg = 'White', font = 'time 8 bold',
    ...
    def floatLambda():
        ...
        t2 = "Lambda igual a :"
        t3 = t2+" "+t
        LbCtg.config(text=t3)
        
        return t1
    
    btnlmbda = Button(SVariable, text="Alterar Valor Lambda", 
                        fg = 'black', bg = 'White', font = 'time 8 bold',
                        command=lambda: floatLambda()) 
    btnlmbda.place(rely=0.85, relx=0.1)
    LbCtg = Label(SVariable, text="Lambda igual a : 0.5",
                        fg = 'black', bg = 'White', font = 'time 8 bold',)
    LbCtg.place(rely=0.95, relx=0.1)
    
    vlr = StringVar(root, value='0.5')
    txtLbda = ttk.Entry(SVariable, width = 15, textvariable = vlr)
    txtLbda.place(rely=0.9, relx=0.1)
    ...
    def janelaAdr():
        ...
        #print(lista_var)
        tvadr.novajanela(SVariable, pesos, lista_var, lstid, lstctg, vlamba, df)          
